...
net = graph_from_data_frame(x[, c('Query', 'Array')])
net = simplify(net)
hub = degree(net)
net = toVisNetworkData(net)
net$nodes$value = hub*2
visNetwork(nodes = net$nodes, edges = net$edges) %>%
visIgraphLayout(layout = 'layout_with_fr') %>%
visEdges(color = list(color = 'gray', highlight = 'blue')) %>%
visNodes(color = list(border = 'black', background = 'black', highlight = 'blue'))
visSave(net, file='sga_network2.html')
##########################
...
library(reshape2)
head(x)
x = dcast(x, Query ~ Array, value.var = 'mean')
dim(x)
View(x)
x[is.na(x)] = 0
View(x)
...
pheatmap(as.matrix(x[, -1]), color = brewer.pal(7, 'RdBu'))
pheatmap(as.matrix(x[, -1]), color = brewer.pal(7, 'RdBu')[7:1])
View(x)
rownames(x) = x$Query
View(x)
...
x = read_csv(tk_choose.files())
x = dcast(x, Array ~ Query, value.var = 'mean')
x[is.na(x)] = 0
View(x)

tsne = Rtsne(as.matrix(x[, -1]))
plot(tsne$Y)
...
dim(x)
View(x)
clus = Mclust(as.matrix(x[, -1]), 2:20)
plot(clus)
summary(clus)
test = cor(t(as.matrix(x[, -1])))
View(test)
...
write.csv(x, 'Heatmap.csv', row.names = FALSE)
